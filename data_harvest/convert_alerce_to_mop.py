...
def convert_lc(args):
    """
    Function to convert Alerce-format forced photometry lightcurve data from the CSV file format
    ...
    alerce_data = load_alerce_lc_csv(args.input_file)

    # Output lightcurves, one per filter, in MOP format
    output_to_mop_format(alerce_data, args.output_file)
...
def load_alerce_lc_csv(input_file):
    """
    Function to load a Alerce forced-photometry lightcurve from a CSV-format input file downloaded
    from the FINK portal https://fink-portal.org/api
    """
    ...
    valid1 = np.where(np.isnan(raw_data['mag_corr']) == False)[0]
    valid2 = np.where(raw_data['e_mag_corr'] < 100.0)[0]
    valid = list(set(valid1).intersection(set(valid2)))

    lc_data = Table([
        Column(name='JD', data=raw_data['mjd'][valid]),
        Column(name='mag', data=raw_data['mag_corr'][valid]),
        Column(name='mag_error', data=raw_data['e_mag_corr'][valid]),
        Column(name='filter', data=raw_data['fid'][valid].apply(lambda x: filter_map[x]))
    ])
