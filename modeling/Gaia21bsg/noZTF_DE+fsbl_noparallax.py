...
# now using an FSBL model for event

fsbl = FSBL_model.FSBLmodel(your_event,fancy_parameters=fancy,parallax=['None',2.45935341e+06])
fit_2 = DE_fit.DEfit(fsbl, telescopes_fluxes_method='polyfit', DE_population_size=20, max_iteration=10000, display_progress=True)

# using best model results from prev no ZTF fit
...
# setting limb darkening using quadratic limb darkening coeffs

your_event.telescopes[0].ld_gamma = 0.503   # i-filter
your_event.telescopes[1].ld_gamma = 0.8371   # g-filter
your_event.telescopes[2].ld_gamma = 0.503    # i-filter
your_event.telescopes[3].ld_gamma = 0.8371   # g-filter
...
if perform_long_fit == True:
    ...
    # Save it
    np.save('results/noZTF_DE+fsbl_noparallax1.npy', fit_2.fit_results['DE_population'])

else:
    # Use the precomputed Differential Evolution (DE) results:
    fit_2.fit_results['DE_population'] = np.load('results/noZTF_DE+fbpl_noparallax1.npy')
    fit_2.fit_results['best_model'] = fit_2.fit_results['DE_population'][346501][0:-1]
    # fit_2.fit_results['best_model'] = [2457205.21, 0.0109583755, 1.78218726, -2.89415218, 0.0475121003, -3.79996021, 2.25499875, 0.0227712230, -0.227
...
plot_lightcurves(fsbl,fit_2.fit_results['best_model'])
plt.show()

plot_geometry(fsbl,fit_2.fit_results['best_model'])
plt.show()
