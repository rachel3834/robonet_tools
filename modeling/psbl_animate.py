...
GM1, GM2, D, cof1, cof2 = 0.01, 0.99, 0.4, 0.1, -0.1
...
def init():
    ...
    ax2.set_xlim(-XLM, XLM)
    ax2.set_yscale('log')
    ax2.grid(True)
    ax2.set_xlabel(r'$x/R_E$', fontsize=12)
    ax2.set_ylabel('Magnification', fontsize=12)
...
def animate(i):
    ...
    # Plot primary lens
    ax1.plot([D], [0], 'ko', markersize=10)
    # Plot secondary lens
    ax1.plot([-D], [0], 'ko', markersize=2)
    if use_criticals:
        ax1.plot(critical_points_x1, critical_points_y1, 'r,', markersize=1)
    ax1.plot(caustic_points_x1, caustic_points_y1, 'b,', markersize=1)
    ...
    ax1.plot(src_x, src_y, 'b.', markersize=1)
    ax1.plot(img_x, img_y, 'g.', markersize=1)
    ax1.grid(True)
    ax1.set_xlim(-XLM, XLM)
    ax1.set_ylim(-2, 2)

    # Magnification plot
    ax2.plot(XSA, ASA, 'b-', lw=0.5)
    ax2.plot(xs, ASA[idx], 'ro')
    ax2.set_yscale('log')
    ax2.set_xlim(-XLM, XLM)
    ax2.grid(True)
...
# Save as MP4
ani.save('binary_microlensing_event2.mp4', writer='ffmpeg', fps=10)
#plt.show()
plt.close(fig)
